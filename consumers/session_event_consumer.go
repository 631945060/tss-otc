...
func (m *Manager) consumeSessionEvents(ctx context.Context) {
	if err := m.client.XGroupCreateMkStream(ctx, services.SessionEventStream, consumerGroup, "0").Err(); err != nil && !strings.HasPrefix(err.Error(), "BUSYGROUP") {
		log.Printf("create Redis consumer group: %v", err)
		return
	}
	for ctx.Err() == nil {
		streams, err := m.client.XReadGroup(ctx, &goredis.XReadGroupArgs{Group: consumerGroup, Consumer: m.consumerName, Streams: []string{services.SessionEventStream, ">"}, Count: 10, Block: time.Second}).Result()
		if err != nil {
		...
		for _, stream := range streams {
			for _, message := range stream.Messages {
				...
				if err := m.client.XAck(ctx, services.SessionEventStream, consumerGroup, message.ID).Err(); err != nil {
